...
ind1 <- sample(2, nrow(preprocessedData), replace=FALSE, prob=c(0.7, 0.3))
ind2 <- sample(2, nrow(preprocessedData), replace=FALSE, prob=c(0.6, 0.4))
ind3 <- sample(2, nrow(preprocessedData), replace=FALSE, prob=c(0.8, 0.2))
...
### 2- Attribute selecting measures
...
8 rules and two decisions make up the tree. One indicates a positive diagnosis, whereas zero indicates a negative diagnosis. 

By following the tree, we will identify two rules: one leads to a positive diagnosis, and the other to a negative one.

 Rule 1: if concave_points_worst is less or equal to 10, and area_worst is less or equal to 5, and concave_points_worst is less or equal to 8, then the diagnosis will be one which is a positive diagnosis.
 
Rule 2: if concave_points_worst is larger than 10, and p=concave_points_mean is larger than 8, then the diagnosis will be zero which is a negative diagnosis.

By following the tree we then can extract all the rules from it.
...
By following the tree, we will identify two rules: one leads to a positive diagnosis, and the other to a negative one.
 ...
 Rule 1: if area_worst is less or equal to 5, and concave_points_worst is less or equal to 8,then the diagnosis will be one which is a positive diagnosis.
 
Rule 2: if area_worst is larger than 5, then the diagnosis will be zero which is a negative diagnosis.
...
15 rules and two decisions make up the tree. One indicates a positive diagnosis, whereas zero indicates a negative diagnosis. 

By following the tree, we will identify two rules: one leads to a positive diagnosis, and the other to a negative one.

 Rule 1: if perimeter_worst is larger than 8, and concave_points_mean is less or equal to 6,and concave_se is larger than 2, then the diagnosis will be one which is a positive diagnosis.
 
Rule 2: if perimeter_worst is larger than 8, and concave_points_mean is larger than 6, then the diagnosis will be zero which is a negative diagnosis.
...
4 rules and two decisions make up the tree. One indicates a positive diagnosis, whereas zero indicates a negative diagnosis. 

By following the tree, we will identify two rules: one leads to a positive diagnosis, and the other to a negative one.

 Rule 1: if perimeter_worst  is less than 9, and concave_points_worst is less than 12, then the diagnosis will be zero which is a negative diagnosis.
 
Rule 2: if perimeter_worst is larger or equal to 9, and texture_worst is larger or equal to 6, then the diagnosis will be one which is a positive diagnosis.
...
5 rules and two decisions make up the tree. One indicates a positive diagnosis, whereas zero indicates a negative diagnosis. 

By following the tree, we will identify two rules: one leads to a positive diagnosis, and the other to a negative one.

 Rule 1: if perimeter_worst  is less than 9, and concave_points_worst is less than 12, and radius_worst is less than 7, then the diagnosis will be zero which is a negative diagnosis.
 
Rule 2: if perimeter_worst is larger or equal to 9, then the diagnosis will be one which is a positive diagnosis.
...
By following the tree, we will identify two rules: one leads to a positive diagnosis, and the other to a negative one.
 ...
 Rule 1: if perimeter_worst  is less than 9, and concave_points_worst is less than 11, then the diagnosis will be zero which is a negative diagnosis.
...
9 rules and two decisions make up the tree. One indicates a positive diagnosis, whereas zero indicates a negative diagnosis. 

By following the tree, we will identify two rules: one leads to a positive diagnosis, and the other to a negative one.

 Rule 1: if concave_points_worst is less or equal to 10, and area_worst is less or equal to 5, and concave_points_worst is less or equal to 8, then the diagnosis will be one which is a positive diagnosis.
 
Rule 2: if concave_points_worst is larger than 10, and concave_points_mean is larger than 8, then the diagnosis will be zero which is a negative diagnosis.
...
6 rules and two decisions make up the tree. One indicates a positive diagnosis, whereas zero indicates a negative diagnosis. 
...
16 rules and two decisions make up the tree. One indicates a positive diagnosis, whereas zero indicates a negative diagnosis. 

By following the tree, we will identify two rules: one leads to a positive diagnosis, and the other to a negative one.

Rule 1: if perimeter_worst is larger than 8, and concave_points_mean is less or equal to 6,and concavity_se is larger than 2, then the diagnosis will be one which is a positive diagnosis.
...
Using the Silhouette method the Optimal number of clusters is 2
...
## Classification for Breast Mass Diagnosis:
...
#### 1- High Accuracy and Specificity:
 The classification model, based on the Decision Tree with the Gini Index, demonstrates high accuracy and specificity. This is crucial in a medical context, especially for breast cancer diagnosis, where correctly identifying benign cases (high specificity) is equally important as correctly identifying malignant cases.
 
#### 2- Balanced Precision and Recall: 
The model achieves a balanced precision and recall, suggesting that it is effective in correctly classifying both malignant and benign instances. This is essential for a diagnostic tool, where false positives and false negatives can have significant consequences.

#### 3- Validation through Gini Index:
The use of the Gini Index for evaluation adds confidence to the results. It indicates that the Decision Tree is effective in creating decision nodes that separate the classes well.

### problem solutions (extracted information):
From the Gini index decision tree (80-20) split, we can determine That pirimeter_worst is the most significant characteristic of the mass because it was the root of the tree, and that 9 is the ideal splitting point for it.

Next, in order of significance, are concave_points_worst and smoothness_worst.

We can extract 4 rules to diagnose the mass from this tree:

Rule 1: if perimeter_worst is less than 9, and concave_points_worst is less than 11, then the diagnosis will be zero which is a negative diagnosis.

Rule 2: if perimeter_worst is less than 9, and concave_points_worst is larger or equal to 11, and smoothness _worst is less than 12, then the diagnosis will be zero which is a negative diagnosis.

Rule 3: if perimeter_worst is less than 9, and concave_points_worst is larger or equal to 11, and smoothness _worst is larger or equal to 12, then the diagnosis will be one which is a positive diagnosis.

Rule 4: if perimeter_worst is larger or equal to 9, then the diagnosis will be one which is a positive diagnosis.

### Overall of Classification for Breast Mass Diagnosis
Achieves high accuracy, precision, sensitivity, and specificity. Well-suited for predicting the nature of breast tumors based on analyzed features. Provides a clear decision boundary for distinguishing between malignant and benign masses.
...
#### 3- High Bcubed Precision and Recall: 
The high precision and recall values for clustering suggest that instances within clusters are more internally consistent, and the algorithm is effective at finding true positive instances.

### problem solutions (extracted information):
By accessing coordinates of a centroid for cluster1 with 2 clusters we come to know that all elements in cluster 2 have higher tumor measurements the the ones in cluster 1, except for the features texture_se and smoothness_se.

### Overall of Clustering for Exploratory Analysis:
Reveals natural groupings or clusters within the dataset. Particularly, k=2 demonstrates well-separated clusters with high internal consistency. Can be valuable for gaining deeper insights into underlying characteristics and relationships among breast masses.

## Overall Assessment
### Decision on Interest and Relevance:
#### 1- Clinical Significance:
 The high performance of the classification model and the quality of the clustering results make the findings highly relevant in a clinical setting. The model can provide accurate predictions for individual cases, while clustering unveils potential patterns and structures within the data.
 
#### 2- Decision Tree Interpretability: 
The Decision Tree provides interpretability, allowing medical professionals to understand the key features influencing the model's decisions. This is crucial for gaining insights into the characteristics associated with cancerous and non-cancerous tumors.

#### 3- Comprehensive Understanding:
 The combination of classification and clustering results offers a more comprehensive understanding of the dataset. It goes beyond individual predictions to reveal broader patterns and relationships among breast masses.

### Conclusion
For the specific problem of breast mass diagnosis, classification is more directly aligned with the primary objective of predicting malignancy based on computed features. However, clustering, especially with k=2, can be complementary for exploratory analysis, providing additional insights into the inherent structures or patterns within the dataset. The two approaches serve different purposes, and their combination may contribute to a more comprehensive understanding of the data.

The obtained mining results are highly interesting and relevant for solving the problem of breast cancer diagnosis. The combination of a well-performing classification model and effective clustering provides a multi-faceted view of the dataset. This information can empower medical professionals with accurate predictions for individual cases and a deeper understanding of underlying patterns within the data, contributing to improved decision-making in clinical practice.


# 10. References

1-	Haldar, A., Pal, P., Ghosh, S., & Pan, S. (2022, September 8). Body Weight Prediction using Recursive Partitioning and Regression Trees (RPART) Model in Indian Black Bengal Goat Breed: A Machine Learning Approach. Indian Journal of Animal Research, Of. https://doi.org/10.18805/ijar.b-4894

2-	Gross, T. (2017, February 1). Abstract C50: Applying the Stages of Change Model to breastfeeding in African-American women: A qualitative study. Cancer Epidemiology, Biomarkers & Prevention, 26(2_Supplement), C50–C50. https://doi.org/10.1158/1538-7755.disp16-c50

3-	Q. (2014, May 25). Decision Tree C5.0 Example. Big Data Knowledge Sharing. https://qizeresearch.wordpress.com/2014/05/25/decision-tree-c5-0-example/

4-	C5.0 Classification Models. (n.d.). https://cran.r-project.org/web/packages/C50/vignettes/C5.0.html#rule-based-models
